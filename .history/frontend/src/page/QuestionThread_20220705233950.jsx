...
export default function QuestionThread() {
  ...
  const HandleThread = (props) => {
    return (
      <div>
        <div><span style={{ fontWeight: 'bold' }}>Category: </span> {props.q.category}</div>
        <div style={{ display: 'flex' }}> <div style={{ fontWeight: 'bold' }}>Discription: </div><div>{props.q.discription}</div></div>
      </div>
    )
  }
  return (

...
const Test = () => {
      ...
      return (
        <Grid container spacing={0}>
          {lls.map((bo, y) => {
            if (bo) {
              return (
                <Grid item xs={2.4} sx={{ display: 'flex', height: '20px' }}> <div style={{ borderRadius: '8px', margin: 'auto', height: '90%', width: '90%', background: week[y], fontWeight: 'bold', color: 'black', fontSize: '6px', textAlign: 'center' }}>{time[x]}</div></Grid>
              )
            } else {
              return (<Grid item xs={2.4} > </Grid>)
            }
  ...
  return (
    <div style={{ width: '300px', height: '300px' }} >
      ...
      <Grid container spacing={0}>
        {l.map((d, i) => <Grid item xs={2.4} sx={{ background: week[i], fontWeight: 'bold', color: 'white', fontSize: '10px', textAlign: 'center' }}>{d}</Grid>)}
      </Grid>

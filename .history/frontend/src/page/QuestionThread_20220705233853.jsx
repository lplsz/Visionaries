...
export default function QuestionThread() {
  ...
  const HandleThread = (props) => {
    return (
      <div>
        <div><span style={{ frontWeight: 'bold' }}>Category: </span> {props.q.category}</div>
        <div style={{ display: 'flex' }}> <div style={{ frontWeight: 'bold' }}>Discription:</div><div>props.q.discription</div></div>
      </div>
    )
  }
  return (

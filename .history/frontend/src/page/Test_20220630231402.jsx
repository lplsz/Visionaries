      ...
      return (
        <Grid container spacing={0}>
          {lls.map((bo, y) => {
            if (bo) {
              return (
                <Grid item xs={2.4} sx={{ display: 'flex', height: '20px', background: week2[x] }}}> <div style={{ borderRadius: '8px', margin: 'auto', height: '90%', width: '90%', background: week[y], fontWeight: 'bold', color: 'black', fontSize: '6px', textAlign: 'center' }}>{time[x]}</div></Grid>
      )
    } else {
      return(<Grid item xs = { 2.4} sx = {{ background: week2[x]}} > </Grid >)
  }

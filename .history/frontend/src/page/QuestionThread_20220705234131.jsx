...
export default function QuestionThread() {
  ...
  const HandleThread = (props) => {
    return (
      <div>
        <div style={{ display: 'flex' }}><div style={{ fontWeight: 'bold', flex: 1 }}>Category: </div> <div style={{ flex: 4 }}>{props.q.category}</div></div>
        <div style={{ display: 'flex' }}> <div style={{ fontWeight: 'bold', flex: 1 }}>Discription: </div><div style={{ flex: 4 }}>{props.q.discription}</div></div>
      </div>
    )
  }
  return (

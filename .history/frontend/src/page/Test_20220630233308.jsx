      ...
      return (
        <Grid container spacing={0}>
          {lls.map((bo, y) => {
            if (bo) {
              return (
                <Grid item xs={2.4} sx={{ display: 'flex', height: '20px', background: week2[y] }}> <div style={{ borderRadius: '8px', margin: 'auto', height: '90%', width: '90%', background: week[y], fontWeight: 'bold', color: 'black', fontSize: '6px', textAlign: 'center' }}>{time[x]}</div></Grid>
              )
  ...
  return (
    <div style={{ width: '400px', height: '300px' }} >
      ...
      <div style={{ width: '100%', display: 'flex' }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' }}> <IconButton size='small' onClick={() => { console.log('hehe'); setDate(getFirstDayOfLastWeek(date)); setI(i + 1) }}><KeyboardDoubleArrowRightIcon size='small' /></IconButton></div>
